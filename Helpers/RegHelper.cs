...
namespace WinFlux.Helpers
{
    internal class RegHelper
    {
        ...
        internal static void DeleteValue(RegistryHive hive, string path, string name)
        {
            try
            {
                var key = SetKey(hive, path);
                if (key != null && KeyExist(hive, path, name))
                {
            ...
            {
                // Silently ignore errors when deleting non-existent values
            }
        }
    }
}
